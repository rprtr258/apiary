...
export const store = ((): Store => {
  ...
  return {
    ...
    movePane(direction: "right" | "left" | "up" | "down"): void {
      ...
      const changed = {
        "right": () => layout.instance?.moveToNextGroup(active),
        "left":  () => layout.instance?.moveToPreviousGroup(active),
        "up":    () => layout.instance?.moveAbove(active),
        "down":  () => layout.instance?.moveBelow(active),
      }[direction]();
      if (changed ?? false)
        updateLocalstorage();
    },
