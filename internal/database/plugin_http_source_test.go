...
func TestParsePetstoreSpec(t *testing.T) {
	...
	require.NoError(t, err, "should parse petstore spec without error")
	require.NotEmpty(t, endpoints, "should parse at least one endpoint")

	// Find the POST /store/order endpoint
	var orderEndpoint *EndpointInfo
	for i := range endpoints {
		if endpoints[i].Path == "/store/order" && endpoints[i].Method == "POST" {
			orderEndpoint = &endpoints[i]
			break
		}
	}
	require.NotNil(t, orderEndpoint, "should find POST /store/order endpoint")
...
func TestGenerateExampleRequestForPetstoreOrder(t *testing.T) {
	...
	httpReq, err := GenerateExampleRequest(*orderEndpoint, serverURL, auth)
	require.NoError(t, err, "should generate example request without error")
	...
	require.True(t, hasContentType, "should have Content-Type header for request with body")
...
func TestGenerateExampleRequestWithRequiredProperties(t *testing.T) {
	...
	auth := AuthConfig{Type: AuthNone}
	httpReq, err := GenerateExampleRequest(endpoint, serverURL, auth)
	require.NoError(t, err, "should generate example request")
...
func TestPetstoreOrderExampleOutput(t *testing.T) {
	// This test shows what actual output is generated for the petstore order
	// and verifies it's not trivial
	// Parse the spec from embedded file
	endpoints, err := ParseSpec(string(petstoreSpec))
	require.NoError(t, err, "should parse petstore spec without error")
...
func TestGenerateExampleRequestExactOutput(t *testing.T) {
	// This test checks for exact output equality to ensure we're generating
	// consistent and correct examples
	// Parse the spec from embedded file
	endpoints, err := ParseSpec(string(petstoreSpec))
	require.NoError(t, err, "should parse petstore spec without error")
...
func TestGenerateExampleRequestConsistency(t *testing.T) {
	...
	httpReq1, err := GenerateExampleRequest(*orderEndpoint, serverURL, auth)
	require.NoError(t, err, "should generate first example request")

	httpReq2, err := GenerateExampleRequest(*orderEndpoint, serverURL, auth)
	require.NoError(t, err, "should generate second example request")

	httpReq3, err := GenerateExampleRequest(*orderEndpoint, serverURL, auth)
	require.NoError(t, err, "should generate third example request")
