...
export async function fetchSpec(sourceRequest: HTTPSourceRequest): Promise<OpenAPI.Document> {
  ...
  case "url":
    ...
    return JSON.parse(await resp.text()) as OpenAPI.Document;
  case "file":
    // spec is inline in specData
    return JSON.parse(sourceRequest.specData) as OpenAPI.Document;
  }
}

export async function parseSpec(specData: OpenAPI.Document): Promise<EndpointInfo[]> {
  const spec = await dereference(specData);
...
function extractRequestBody(requestBody: RequestBodyObject | ReferenceObject): RequestBodyInfo {
  ...
  if (body?.content !== undefined) {
    for (const [mediaType, mediaTypeObj] of Object.entries(body.content)) {
      content[mediaType] = {
        schema: (mediaTypeObj as MediaTypeObject).schema as JSONSchema,
        example: undefined,
      };
    }
  }
  ...
  for (const [code, response] of Object.entries(responses)) {
    ...
    if (respObj.content !== undefined) {
      for (const [mediaType, mediaTypeObj] of Object.entries(respObj.content)) {
        content[mediaType] = {
          schema: mediaTypeObj.schema as JSONSchema,
          example: undefined,
        };
      }
    }
